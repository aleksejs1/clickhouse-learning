...
async function drawChart(eventId, similarBy, field) {
    const resp = await fetch(`/api/events/${eventId}/chart/${field}?similar_by=${similarBy}`);
    if (!resp.ok) {
...
if (root) {
    const { eventId, similarBy, fields } = root.dataset;
    for (const field of fields.split(',')) {
        drawChart(eventId, similarBy, field).catch(console.error);
    }
}
