...
**Статус (реализовано без доступного ключа):** весь код написан. Проверено
без ключа: I0 (деградация 503, `ai_enabled`), I1 (промпт собирается, байт-в-байт
воспроизводим), I4.3 (панели скрыты, проект работает), I4.4 (README). Путь кода
I2 проверен до Anthropic API включительно — с фиктивным ключом запрос доходит и
возвращает 401 (сборка промпта, JSON-схемы и HTTP-слой работают; попутно
доустановлен PSR-17 `nyholm/psr7`, без которого SDK не шлёт запросы). Разбор
ответа модели, петля ретрая (I2.3), браузерный round-trip чата (I3) и приёмка
I4.1/I4.2 требуют реального ключа — отмечены галочкой не будут, пока не прогнаны.
