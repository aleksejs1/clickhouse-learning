...
Ещё в ответе есть `queries` — список выполненных SELECT-ов со статистикой из
HTTP-заголовка `X-ClickHouse-Summary` (ClickHouse шлёт его с каждым ответом):
`sql`, `params`, `read_rows`, `read_bytes`, `total_rows` (план чтения после
отсечения по индексу), `elapsed_ms`. Собирает их сервис `ClickHouse` (метод
`queryLog()`), UI показывает под каждым графиком строку «прочитано N строк ·
мс · SQL», по клику раскрываются сами запросы. Аналогичная строка под
таймлайном на главной наглядно показывает выигрыш MV: тысячи строк вместо
сотен миллионов.
...
## 4. `GET /api/map` — гео-распределение групп

Параметры и валидация те же, что у `/api/chart` (`similar_by`, `value`,
`from`/`to`). События агрегируются в ячейки сетки ~1×1 км (округление
координат до 2 знаков), максимум 4000 самых наполненных ячеек:

```json
{
  "similar_total": 387187,
  "other_total": 9500000,
  "cells": [ { "lat": 56.92, "lon": 24.14, "similar": 14, "other": 3841 } ],
  "queries": [ "…как в /api/chart" ]
}
```

Карта на странице события красит ячейку янтарным, если доля «похожих» в ней
выше их доли в целом (`similar/similar_total > other/other_total`) — маршрутные
группы рисуют свой коридор. Радиус кружка — логарифм наполненности.

## 5. Страницы (HTML, Twig)
...
1. **Карточка события** — все поля в две колонки (простой `<dl>`), плюс
   вычисляемое `distance_to_riga_km` — гео-функция ClickHouse
   `greatCircleDistance(lon, lat, 24.10, 56.95)` прямо в SELECT.
1a. **Карта** (Leaflet с CDN, тайлы OpenStreetMap) — гео-распределение групп
   из `/api/map`, см. §4. Маркер — открытое событие.
