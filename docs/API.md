...
Зачем: сортировочный ключ таблицы начинается с `toStartOfHour(event_time)`,
поэтому такой фильтр отсекает гранулы по первичному индексу — на десятках
миллионов строк графики ускоряются на порядок. Страницы прокидывают период
насквозь: таблица → ссылки строк → страница события → chart-запросы.

## Сэмплирование (эндпоинты `/api/chart`, `/api/map`, страница `/anomalies`)

Опциональный query-параметр `sample` (`0.1` или `0.01`, белый список в
`App\Sampling`) добавляет в запросы `SAMPLE 0.1` — читается ~десятая часть
строк по ключу `SAMPLE BY xxHash32(event_id)`. Счётчики сервер домножает на
1/rate, ответ содержит поле `sample`; UI показывает «N≈». Средние значения
не искажаются. Подробности и грабли выбора ключа — DATA_MODEL.md §8.
...
### `GET /anomalies` — автопоиск аномалий

BubbleUp без выбора события. Один проход по таблице собирает средние всех
метрик для всех групп всех измерений:

```sql
SELECT dim1, …, dim10, count() AS n, avg(m1), …, avg(m11)
FROM events [WHERE период]
GROUP BY GROUPING SETS ((dim1), (dim2), …, (dim10))
```

`GROUPING SETS` — несколько независимых GROUP BY за одно сканирование; в
каждой строке результата заполнено ровно одно измерение, остальные пустые.
Второй запрос — глобальные `avg`/`stddevPop` метрик. PHP считает отклонение
каждой группы в сигмах (`|group_avg − global_avg| / stddev`), фильтрует мелкие
группы (N < 1000), оставляет ≤3 значений на пару измерение×метрика (иначе
структурные корреляции вроде «все фургоны экономичнее» вытесняют всё) и
показывает топ-60. Ссылка «исследовать» ведёт на `GET /explore/{dim}/{value}` —
редирект на страницу свежего события группы с `similar_by={dim}`.

Ожидаемо: наверху и заложенные аномалии, и «структурные» корреляции (скорость 0
у стоящих машин) — отличать их и есть работа аналитика.
