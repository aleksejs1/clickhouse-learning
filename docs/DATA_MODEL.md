...
## 6. Кодеки, bloom-индекс, TTL

Всё ниже — из замеров на реальных данных проекта (156M строк), а не из общих
соображений. Методика: тестовая таблица `codec_test` на 3M строк с
кандидатами-кодеками, `OPTIMIZE FINAL`, сравнение `ratio =
uncompressed/compressed` по `system.parts_columns` с основной таблицей.

### Кодеки сжатия (поверх LZ4)

| Колонка | LZ4 | С кодеком | Вердикт |
|---|---|---|---|
| `event_time` (DoubleDelta) | ×10.2 | **×68.6** | отсортирована по ключу, дельты крошечные |
| `cargo_weight_kg` (T64) | ×1.1 | **×2.1** | T64 обрезает пустые старшие биты |
| `odometer_km` (T64) | ×1.0 | **×1.7** | |
| `engine_rpm` (T64) | ×1.0 | **×1.4** | |
| `harsh_events_cnt` (T64) | ×2.0 | **×3.3** | |
| Float32-метрики (Gorilla) | ×1.4–1.8 | ×0.9–1.3 | **проигрыш** — Gorilla хорош для медленно меняющихся рядов, а наши значения случайны от строки к строке |
| `event_id` (UUID) | ×1.0 | — | случайные UUID несжимаемы, кодеки бессильны |

Вывод для джуна: кодеки не «включай все подряд», а меряй на своих данных —
Gorilla здесь сделал бы таблицу *больше*.

### Bloom-индекс для точечного поиска

`event_id` — второй в `ORDER BY`, поэтому `WHERE event_id = ...` — полный скан
(156M строк, ~140 мс). Skip-индекс `INDEX idx_event_id event_id TYPE
bloom_filter GRANULARITY 4` позволяет пропускать гранулы, где значения точно
нет. Это не «индекс» в смысле B-tree: он лишь фильтрует, какие блоки читать.

### TTL и партиции

- `TTL event_time + INTERVAL 90 DAY` — события старше 90 дней удаляются сами
  при фоновых слияниях (для форсирования: `OPTIMIZE TABLE events FINAL`).
- Партиции по месяцам позволяют удалять данные мгновенно и бесплатно:
  `ALTER TABLE events DROP PARTITION '202606'` (осторожно: удаляет месяц данных
  безвозвратно). Список партиций: `SELECT partition, sum(rows), formatReadableSize(sum(bytes_on_disk))
  FROM system.parts WHERE table='events' AND active GROUP BY partition`.

При изменении схемы на живой базе: `ALTER MODIFY COLUMN ... CODEC` / `ADD INDEX`
меняют только метаданные — старые куски переписываются лишь при слияниях или
принудительном `OPTIMIZE TABLE events FINAL`.

## 7. Гео-поля `lat` / `lon`

География: маршруты — лучи из главного города региона в Ригу. Точка события =
случайная позиция на линии + поперечный шум:

| Регион | Стартовый город | Координаты |
|---|---|---|
| riga | Бауска | 56.41, 24.19 |
| kurzeme | Лиепая | 56.51, 21.01 |
| latgale | Даугавпилс | 55.87, 26.52 |
| vidzeme | Валмиера | 57.54, 25.43 |
| zemgale | Елгава | 56.65, 23.71 |
| (конец всех маршрутов) | Рига | 56.95, 24.10 |

`lat = start_lat + t·(56.95 − start_lat) + шум·0.06`, аналогично `lon`
(шум ±0.05°/±0.10°). Генератор считает это в PHP для новых событий.

Гео-поля не входят в `Schema::METRICS` (гистограммы широты бессмысленны) —
для них константа `Schema::GEO` и карта (`/api/map`).

## 8. Сэмплирование

Таблица создана с `SAMPLE BY xxHash32(event_id)`; `SELECT ... FROM events
SAMPLE 0.1` читает ~10% строк и даёт приблизительный, но мгновенный ответ.
UI: селектор «сэмпл» на странице события и в автопоиске аномалий;
счётчики домножаются на 1/rate (в легендах — «N≈»), средние (avg) в
масштабировании не нуждаются.

**Главный урок — позиция ключа сэмплирования.** Первая версия ключа
`ORDER BY (event_time, xxHash32(event_id))` формально работала (`SAMPLE 0.1`
возвращал 10% строк), но замер `read_rows` показал: читались **все** гранулы,
сэмпл лишь фильтровал строки на CPU и был *медленнее* полного скана. Причина:
в одной секунде `event_time` ~десятки строк — каждая гранула (8192 строки)
покрывает почти весь диапазон хэша, пропускать нечего. Рабочий ключ —
`ORDER BY (toStartOfHour(event_time), xxHash32(event_id))`: внутри часа сотни
тысяч строк отсортированы по хэшу, и сэмпл пропускает гранулы по-настоящему.

Цена: сырой `event_time` выпал из ключа, поэтому top-N запросы пишутся как
`ORDER BY toStartOfHour(event_time) DESC, event_time DESC` (тот же порядок,
но ClickHouse читает в порядке индекса и останавливается на LIMIT).

**Учебный трюк — бэкфил через DEFAULT.** Колонки добавлялись на живую таблицу
со 156M строк так: `ALTER TABLE events ADD COLUMN lat Float32 DEFAULT
<та же формула на SQL: transform(region, …) + cityHash64(event_id)…>`.
DEFAULT-выражение может ссылаться на другие колонки, старые куски вычисляют
его при чтении «виртуально» (мгновенный ALTER), а `ALTER TABLE events
MATERIALIZE COLUMN lat` затем запекает значения на диск. Детерминированный
«рандом» — хэш от `event_id`. Новые вставки от генератора кладут свои значения
и DEFAULT не используют.
